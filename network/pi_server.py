...
def identify_person(array, model, window, norm_stats=None):
...
class ClientHandler:
    ...
    async def handle_client(self, reader, writer):
        ...
        payload_size = self.window * joints * 3 * 4
        while True:
            try:
...
def main():
    ...
    parser.add_argument('-w', '--window', type=int, default=10,
                        help='Window size for the input data.')
    args = parser.parse_args()
    
    server = ClientHandler(args.model, './normalization_stats.json', args.drop_prob, args.window)

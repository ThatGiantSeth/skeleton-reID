...
def identify_person(numpy_array):
    ...
    # Apply normalization (normalize to [0,1] based on this sample's own min/max)
    numpy_array = normalize_skeleton(numpy_array)
...
model.load_state_dict(torch.load('./skeleton_model_best.pth', map_location='cpu'))

expected_person = 'antonio'
# Example usage with data from data_val
# Load a sample from validation data
sample_data = np.load(f'./data_val/{expected_person}_standing1000.npy')
# Select a random window of 50 frames

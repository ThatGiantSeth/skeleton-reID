...
def compute_normalization_stats(data, root_joint=0):
    ...
    channel_range = np.maximum(channel_maxs - channel_mins, 1e-6)

    return {
        "root_joint": int(root_joint),
        "mins": channel_mins.astype(np.float32).tolist(),
        "range": channel_range.astype(np.float32).tolist(),
    }


def normalize_skeleton(data, root_joint=0, stats=None):
    if stats is not None:
        root_joint = int(stats.get("root_joint", root_joint))
    ...
    if stats is None:
        ...
        channel_mins = np.asarray(stats["mins"], dtype=np.float32)
        channel_range = np.asarray(stats["range"], dtype=np.float32)

    # normalize x,y,z to [0,1]
    min_bc = channel_mins.reshape(1, 1, -1)
    range_bc = channel_range.reshape(1, 1, -1)
    data = (data - min_bc) / np.maximum(range_bc, 1e-6)
